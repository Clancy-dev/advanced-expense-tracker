...
import { updateBudgetItem } from "@/actions/budget"
...
export default function EditBudgetItemForm({ budgetItem }: EditBudgetItemFormProps) {
  ...
  async function onSubmit(data: BudgetItemFormProps) {
    try {
      ...
      console.log("Updating budget item with data:", data)

      const result = await updateBudgetItem(budgetItem.id, data)

      if (!result.success) {
        throw new Error(result.error || "Failed to update budget item")
      }

      console.log("Budget item updated successfully:", result.data)
      toast.success("Budget item updated successfully")

      router.push("/dashboard/budget")
      router.refresh()
    } catch (error) {
      console.error("Error updating budget item form:", error)
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <Card className="border border-purple-100 shadow-md">
        <CardHeader className="bg-gradient-to-r from-purple-50 to-indigo-50">
          <CardTitle className="text-purple-700">Edit Budget Item</CardTitle>
          <CardDescription>Update your budget item details.</CardDescription>
        </CardHeader>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="flex justify-end space-x-4 pt-2">
              ...
              <Button type="submit" disabled={isSubmitting} className="bg-purple-600 hover:bg-purple-700 text-white">
                {isSubmitting ? "Updating..." : "Update Budget Item"}
              </Button>
            </div>
          </form>
        </CardContent>
